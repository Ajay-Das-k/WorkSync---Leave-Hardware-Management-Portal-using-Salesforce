...
export default class LeaveEmployeeDashboard extends LightningElement {
  ...
  @api showHardwareOnly = false;
  @api hideHomeAndLeaves = false;
  ...
  wiredAssignedHardware(result) {
    ...
    if (result.data) {
      this.myAssignedHardware = result.data.map((hw) => {
        const rawPercent =
          hw.Life_Span_Consumed_Percentage__c != null
            ? hw.Life_Span_Consumed_Percentage__c
            : 0;
  ...
  wiredHardwareTickets(result) {
    ...
    if (result.data) {
      this.myHardwareTickets = result.data.map((ticket) => ({
        ...ticket,
        hardwareName: ticket.Hardware__r
          ? ticket.Hardware__r.Name
          : "Deleted Hardware",
        hardwareImageUrl: ticket.Hardware__r
          ? ticket.Hardware__r.Image_URL__c
          : ""
      }));
    } else if (result.error) {
  ...
  get profileCompletion() {
    ...
    fieldsToCheck.forEach((field) => {
      if (
        ...
        String(this.profileData[field]).trim() !== ""
      ) {
        filledCount++;
      }
    });
    ...
    getContactDetails({ contactId: this.currentUser.contactId })
      .then((result) => {
        if (result.success) {
          ...
          console.log(
            "Profile Data Loaded with Gender:",
    ...
    if (data) {
      this.myLeaves = data.map((leave) => {
        ...
        if (displayId && displayId.length >= 15 && displayId.startsWith("a")) {
          displayId =
            "LR-" + leave.Id.substring(leave.Id.length - 6).toUpperCase();
        }
      ...
      this.pendingCount = data.filter(
        (l) =>
          l.Status__c === "Submitted" || l.Status__c === "Pending HR Approval"
      ).length;
  ...
  async handleDeleteRequest(event) {
    ...
    if (!leaveId) {
      await LightningAlert.open({
        message:
          "Error: Could not identify the leave request to delete (ID missing).",
        theme: "error",
        label: "Error"
      });
      return;
  ...
  async submitHardwareTicket() {
    ...
    try {
      ...
      this.showToast(
        "Success",
        "Hardware ticket submitted successfully.",
        "success"
      );
  ...
  async handleDeleteTicket(event) {
    ...
    if (result) {
      try {
        ...
        this.showToast(
          "Success",
          "Hardware ticket deleted successfully.",
          "success"
        );
        refreshApex(this.wiredHardwareTicketsResult);
      } catch (error) {
